...
- [ ] **`WelcomeView` hand-rolls its own copy of the wordmark** (`WelcomeView.swift:101-112`) instead of using `WordmarkView` — found 2026-09-21 when an accessibility fix to the shared component silently failed to reach the Welcome screen. Second instance of the same shape as the item above. Note the copy is **missing `.tracking(-1.1)`**, so the mark renders with different letter spacing on Welcome than everywhere else — a live visual inconsistency, not only a maintenance smell. Consolidating changes Welcome's wordmark spacing visibly, so look before merging.
...
- [x] **Accessibility sweep** — **DONE 2026-09-21**, run with Accessibility Inspector against the app on an iPhone 17 Pro simulator. 18 fixes across 10 files. The pre-existing work (illustrations hidden, Lottie hidden, `SoftBackground` hidden, both back buttons and "Try another" labelled, hero photo labelled) all held up. What running it found that reading the code had not:
  - **Every decorative glyph spoke its asset filename** — "glyph-location-pin, Image", "glyph-walking-person, Image". `AppGlyph.image()` applies no accessibility markup, so an unlabelled `Image` falls back to the asset name. Fixed with `.accessibilityHidden(true)` at 9 call sites (app bar pin, Welcome pin, walking person, cutlery, Result pin, two stars, No Results pin, café illustration).
  - **The wordmark read as "hungry" then "now"** — three siblings in an `HStack` (two `Text` runs plus a `Circle` dot). Now `.accessibilityElement(children: .ignore)` + label "hungrynow". **`WelcomeView` has its own hand-rolled copy of the wordmark** rather than using `WordmarkView`, so the shared fix did not reach it and it needed the same change separately — see the duplication item below.
  - **The facts card read its columns interleaved** — "810 m", "4,5", "away", "415 reviews": every value first, then the captions, so each label followed the wrong number. Each column is now one element reading "810 m, away".
  - **A rating read as two numbers.** In a comma-decimal locale `ratingValue` renders "4,3", which VoiceOver reads as a list: "four, three". `FactFormatter.spokenRating` now speaks "Rated 4 point 3 out of 5" — locale-aware via `Locale.current.decimalSeparator`, taking the already-formatted string so the column and the speech share one rounding rule. "out of 5" also restores the scale the star glyph carried before it was hidden.
  - **Both "·" separators in the specialties row were spoken** as stops. Hidden.

  **Verified by reading labels in the Inspector, not by ear** — the Inspector's speak button truncates each utterance when you step to the next element, which is the tool's behaviour and not a defect. Welcome, Home and Result were stepped through element by element; the remaining screens were fixed by source inventory (all 12 asset `Image` sites audited directly) and spot-checked.
- [x] **Longest-real-string pass** at the largest accessibility Dynamic Type size — **DONE 2026-09-18** on SE/16.4, logged in full as "Dynamic Type pass: Cancel was unreachable on screen 05". Four real defects found and fixed: `SearchingView` stranded the user with Cancel pushed off-screen (now pinned while content scrolls); `PrimaryButton`/`SecondaryButton`/`GhostButton` truncated their own labels to "Directi…" (now 2 lines, 0.6 floor); the wait note capped at `xxxLarge`. Deliberately left truncating: the app-bar location chip, which shares a fixed-height row across six screens. Not ours to fix: screen 06A's stacked stars come from Google's own `GMSPlaceDetailsView`. **Ticked 2026-09-21** — the line stayed unchecked for three days after the work was done and was restated twice as open. Note the original wording also named long real Places names and the Indonesian translation; the logged pass covers the slider and real data, not a translated build, which does not exist yet
