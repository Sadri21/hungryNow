...
## 2026-09-18 — MVP declared complete

All five ship-blockers in `checklist.md` → "Next up" are closed. Marking it explicitly because two of them were already done and the checklist still said otherwise — item 3 read "Still not a repo" while the repo had been public for a day, and item 5 was unstruck although the log recorded the pass finishing. The checklist was trailing reality by a day in both directions.

**What "MVP complete" means here:** the app runs correctly end to end on iPhone SE / iOS 16.4 — the oldest OS the deployment target claims — with the compliance blocker (photo attribution) verified on device, the repo public with README/screenshots/LICENSE, and the office identity rewritten out of the author metadata. Everything still unchecked is polish, mockup-side CSS, or icon-composer work.

**The one real gap:** the accessibility *inspector* / VoiceOver-label sweep. It is a separate checklist item from the verification pass, and conflating the two is what made the MVP look less finished than it was. Worth closing before a buyer looks, since a buyer may well check it — but it does not block a portfolio release.

**The gate that just opened.** "After the MVP works" is now the active section. Three of its four lines predate the app and contradict later decisions: the gigs lead with Android against `ios-vs-android.md`'s standing iOS-first call, they describe bolting a chatbot onto an existing app rather than what was actually built, and they name the office app as portfolio proof — the same office identity deliberately scrubbed from this repo hours earlier. Annotated in place rather than rewritten, because the replacement pitch is a business decision, not a documentation fix. `ai-scalable-pitch.md` already holds the better framing (the "AI-Ready Codebase" add-on, with this vault as the demonstration).

**Pattern worth naming, fourth instance this month:** every one of these was a line that was true when written and expired silently — screen 08 reusing `FactRow`, the `Rp Rp` tier decision, "still not a repo", the Fiverr gigs. None was wrong at the time. The CLAUDE.md rule to verify a task description against the code before trusting it is doing real work, and it applies to the business sections too, not just the technical ones.

## 2026-09-21 — Accessibility sweep run and closed

The last quality gap before a buyer looks is shut. 18 fixes across 10 files, all found by running Accessibility Inspector against the app rather than by reading the code — which is the point of the item, and the code had looked fine.

**What the pre-existing work got right.** Every illustration hidden, the Lottie hidden, `SoftBackground`'s circles hidden, both back buttons and the icon-only "Try another" labelled, the hero photo labelled `"Photo of \(hero.name)"`. None of that needed touching. The port-time accessibility work was real.

**What it missed, in increasing order of interest:**

1. **Decorative glyphs spoke their asset filenames.** "glyph-location-pin, Image". `AppGlyph.image()` applies no accessibility markup at all, so every unlabelled `Image` falls back to the asset name — internal naming read aloud to the user. Nine call sites, all `.accessibilityHidden(true)`. Worth noting the helper *could* have been fixed centrally, but it takes a `size`/`color` signature used at ~30 sites and every glyph found was decorative, so hiding at the 9 that mattered was the smaller diff.

2. **The wordmark fragmented into "hungry" / "now".** Two `Text` runs plus a decorative `Circle`, three siblings in an `HStack`. `.accessibilityElement(children: .ignore)` + an explicit "hungrynow" label — ignore rather than combine, because combine would stitch the children's own text and leave the dot as a stray child.

3. **The facts card read its columns interleaved.** "810 m" → "4,5" → "away" → "415 reviews". Every value first, then every caption, so each label landed against the wrong number. This is a *different class of defect* from the first two: every element correctly labelled, assembled into a sentence that means nothing. It cannot be found by reading code and it cannot be found by auditing labels one at a time — only by listening to the order.

4. **A rating read as two numbers.** `FactFormatter.ratingValue` correctly renders "4,3" in a comma-decimal locale — the file's own doc comment already knew this and it is right for display. But VoiceOver reads the comma as a list separator, so a 4.3-star restaurant is announced "four, three". Added `FactFormatter.spokenRating`, which speaks "Rated 4 point 3 out of 5". Two decisions inside it: it reads `Locale.current.decimalSeparator` rather than assuming a comma, and it takes the **already-formatted string** rather than the `Double`, so the column and the speech cannot drift to two rounding rules. "out of 5" is a bonus — the scale was carried visually by the star glyph, which we had just hidden.

Considered and rejected: spoken forms for distance and price too. "Eight hundred ten metres away" is no clearer than what it says now, and each paired string is a second place that can drift from the formatter. This repo already has two duplicate-renderer bugs on record; eight more paired strings invites a third.

**A gotcha that cost most of the session.** Accessibility Inspector will not drive an **iOS 16.4 simulator under Xcode 26.4.1** — the crosshair arms, hovering highlights nothing, the first click disarms it. Permissions were not the cause (though the Inspector does need adding to Privacy & Security → Accessibility by hand; it is not there by default, and its path is `/Applications/Xcode.app/Contents/Applications/Accessibility Inspector.app` — note *no* `Developer` segment, unlike Simulator.app). The same setup works immediately against a current-iOS simulator. **Do label sweeps on a modern runtime; labels are properties of the Swift source and do not vary by OS version.** Layout, glyph rendering and Dynamic Type still want the SE — that is where 16.4 tells the truth and a modern sim lies.

**The pattern, fifth instance this month.** The wordmark fix to the shared `WordmarkView` silently failed to reach the Welcome screen, because `WelcomeView` hand-rolls its own copy of the mark. Same shape as `AsymmetricRoundedRectangle` / `PurposeCardShape`, and as `FactRow`. In this codebase a component-level fix cannot be assumed to cover every screen — grep for the duplicate before believing the fix landed. The copy is also missing `.tracking(-1.1)`, so the wordmark has been rendering with different letter spacing on Welcome than everywhere else. Added to the checklist as its own item.

**Scope, honestly.** Welcome, Home and Result were stepped through element by element. The remaining screens were fixed by source inventory — all 12 asset `Image` sites audited directly — and spot-checked rather than walked. Also: the Inspector's speak button truncates each utterance when you step to the next element. That is the tool, not the app; read the Label field, and save real VoiceOver for a flow-level pass.

Still open from the same section: the **longest-real-string pass** at the largest Dynamic Type size, which is layout work and wants the SE.
